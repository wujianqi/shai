...
export default class Division {
    ...
    constructor(code: string | number, datas: RegionData) {
        const cc = typeof code === 'number' ? String(code) : code;
   ...
   */
    public getData(index: number = 0): RegionInterface {
        const d = this.code;
    ...
    private getCode(): void {
        ...
        const c = String(util.getItem(prc)) + '0000',
    ...
    private getPfCode(): string {
        ...
        const c = util.getItem(this.prefectures),
    ...
    private getCtCode(): string {
        ...
        const c = util.getItem(this.countys),
    ...
    private findRegion(): void {
        ...
        let hasFinded = false;
        const cd2 = this.divisionCode.slice(0, 2);
        const cd4 = this.divisionCode.slice(0, 4);
        ...
        // 当前为省级
            ...
            for (const key in this.regions) {
                if (this.regions.hasOwnProperty(key)) {
                    if (new RegExp('^' + cd2 + '\\d{2}0{2}').test(key) && !/0{4}$/.test(key)) {
                        hasFinded = true;
                        this.prefectures.push(key);
                    } else if (new RegExp('^' + cd2 + '\\d{4}').test(key) && !/0{2}$/.test(key)) {
                        hasFinded = true;
        ...
        // 当前为市级
            ...
            for (const key in this.regions) {
                if (this.regions.hasOwnProperty(key)) {
                    if (new RegExp('^' + cd4 + '\\d{2}').test(key) && !/0{2}$/.test(key)) {
                        hasFinded = true;
                        this.countys.push(key);
                    } else if (hasFinded) { break; }
                }
