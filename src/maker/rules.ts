...
export const rules = {
    md5: (
        is16b: boolean = false,
        arg: string = String(new Date().getTime()),
    ...
    price: (
        ...
        let n;
        const d = util.getNumber(low, high, 2),
            r1 = /\d{1,3}(?=(\d{3})+$)/g,
            r2 = /^(-?)(\d+)((\.\d+)?)$/;

        if (arg3) {
            n = String(d).replace(r2, (s: string, s1: string, s2: string, s3: string) => 
                s1 + s2.replace(r1, '$&,') + s3);
        }
        else { n = String(d); }
        return n;
    ...
    enName: () =>
        util.getItem(names.eMaleName.concat(names.eFemaleName)) + (geth(0, rules.esurname()) as string),
    enMaleName: () =>
        util.getItem(names.eMaleName) + (geth(0, rules.esurname()) as string),
    enFemaleName: () =>
        util.getItem(names.eFemaleName) + (geth(0, rules.esurname()) as string),
    surname: () => (history[1] = util.getItem(names.cSurname)),
    cnName: () => (geth(1, rules.surname()) as string) +
        util.getItem(names.cMaleName.concat(names.cFemaleName)),
    cnMaleName: () => (geth(1, rules.surname()) as string) + util.getItem(names.cMaleName),
    cnFemaleName: () =>
        (geth(1, rules.surname()) as string) + util.getItem(names.cFemaleName),
    enState: () => {
        const index = util.getInt(0, names.eStates.length - 1);

        if (!history[2]) { history[2] = index; }
        return names.eStates[geth(2, index) as number];
    },
    cnState: () => {
        const index = util.getInt(0, names.cStates.length - 1);
    ...
    file: (...exts: string[]) => {
        let ext;
        const name = new RandExp(/[a-zA-Z][-_]*[a-zA-Z0-9]{1,9}/).gen();
